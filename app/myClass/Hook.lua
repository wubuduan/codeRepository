...
function Hook:ctor(params)
	self.funcL = params.funcL --钩子发射回调
	self.funcBB = params.funcBB --钩子回收开始回调
	self.funcBE = params.funcBE --钩子回收结束回调
	self:init()
end
...
function Hook:hookLaunch()
	...
	self._hook:setAnchorPoint(ccp(0.5, 0.7))
end
...
function Hook:hookBack()
	...
	local move = CCMoveTo:create(time, ccp(display.cx-5, display.cy+60))

	local seq = getSequence({move,CCCallFunc:create(function( )
		self._hook:setAnchorPoint(ccp(0.5, 1))
		self:startToRotation()
		if self.funcBE then
...
function ac_dly_fuc(ob,tk,fuc)
	...
	local ac_fuc = CCCallFunc:create(fuc)
